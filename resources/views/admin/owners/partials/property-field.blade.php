...
<div class="bg-light rounded-3 p-6 border border-dashed mb-8">
    <div class="d-flex align-items-center mb-5">
        <span class="symbol symbol-45px symbol-light-primary me-4">
            <span class="symbol-label">
                <i class="la la-user-tie fs-2 text-primary"></i>
            </span>
        </span>
        <div>
            <h3 class="fs-5 fw-bold mb-1">{{ __('owners.sections.property.title') }}</h3>
            <p class="text-muted mb-0">{{ __('owners.sections.property.subtitle') }}</p>
        </div>
    </div>
        ...
        <div class="row">
            <div class="col-12 col-lg-8">
                <x-admin.select-solid
                    name="property_id"
                    :label="__('owners.assign_property')"
                    :options="$properties"
                    :value="$value"
                    :help="__('owners.sections.property.help')"
                    required
                />
            </div>
        </div>
    @else
        ...
        <div class="alert alert-light-warning d-flex align-items-start gap-3 mb-6">
            ...
            <div>
                <span class="fw-semibold d-block">{{ __('owners.sections.property.locked_warning') }}</span>
                <span class="text-muted small">{{ __('owners.sections.property.locked_hint') }}</span>
            </div>
        </div>
        <x-admin.input-solid
