...
@section('title', __('locations.edit_state'))
...
@section('breadcrumbs')
    ...
    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
        ...
        <li class="breadcrumb-item text-muted">{{ __('locations.edit_state') }}</li>
    </ul>
@endsection

@section('content')
    <x-admin.form-card :title="__('locations.edit_state')" :action="route('admin.states.update', $state)" method="PUT" :back="route('admin.states.index')">
        ...
        <x-admin.select-solid
            ...
            :value="old('governorate_id', $state->governorate_id)"
            required
            data-role="governorate-select"
        />
        <x-admin.input-solid name="name" :label="__('locations.state_name')" :value="old('name', $state->name)" required />
    </x-admin.form-card>
@endsection
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const initialGovernorate = @json(old('governorate_id', $state->governorate_id));
